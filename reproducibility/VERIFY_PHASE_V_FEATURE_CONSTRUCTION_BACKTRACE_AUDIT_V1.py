...
for x in d['candidate_files']:
 p=ROOT/x['path']; assert p.exists(); assert hashlib.sha256(p.read_bytes()).hexdigest()==x['sha256']; assert x['markers']
...
print('PASS: Phase V feature construction backtrace audit v1 provenance, inventory, candidate, and non-imputation invariants verified.')
