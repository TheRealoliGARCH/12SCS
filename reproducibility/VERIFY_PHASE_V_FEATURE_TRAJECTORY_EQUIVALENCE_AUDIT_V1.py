...
for key in ['exact_equivalence','central_symmetry']:
 classes=d[key+'_classes']; count=d[key+'_class_count']; assert len(classes)==count; flat=[x['feature_index'] for g in classes for x in g]; assert sorted(flat)==list(range(55)); assert len(flat)==len(set(flat)); assert d[key+'_reduction']==55-count
