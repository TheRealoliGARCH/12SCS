...
OUT=ROOT/'results'/'phase_v_feature_construction_backtrace_audit_v1.json'
...
for name in roots:
 ...
 for f in sorted(p.rglob('*')):
  if not f.is_file() or f.suffix not in {'.py','.json','.csv','.md','.txt'} or f.resolve() in exclude: continue
...
out={'status':status,'source_path':str(SRC.relative_to(ROOT)),'source_sha256':hashlib.sha256(raw).hexdigest(),'n_features':54,'search_scope':roots,'excluded_paths':[str(OUT.relative_to(ROOT))],'candidate_files':hits,'identity_candidate_count':len(identity_files),'interpretation':'repository-level textual provenance candidates are inventoried only; semantic identity is not asserted unless an explicit feature-index-to-source-variable mapping is present'}
