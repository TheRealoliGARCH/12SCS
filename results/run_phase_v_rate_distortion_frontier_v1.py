...
raw=SRC.read_bytes(); d=json.loads(raw); Y=[f['six_mode_coordinates'] for f in d['features']]; n=len(Y); p=6; mu=[sum(r[j] for r in Y)/n for j in range(p)]; X=[[r[j]-mu[j] for j in range(p)] for r in Y]
...
front=[]
for k in [5,4,3,2,1]:
 R=[[sum(X[i][j]*Q[m][j] for j in range(p))*Q[m][l] for m in range(k)) for l in range(p)] for i in range(n)]
 D=[[X[i][j]-R[i][j] for j in range(p)] for i in range(n)]; front.append({'dimension':k,'retained_energy':sum(lam[:k])/total if total else 0.0,'residual_energy':sum(lam[k:])/total if total else 0.0,'residual_frobenius_norm':math.sqrt(sum(x*x for r in D for x in r)),'max_abs_coordinate_distortion':max(abs(x) for r in D for x in r)})
