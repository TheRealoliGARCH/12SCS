...
ROOT=Path(__file__).resolve().parents[1]; SRC=ROOT/'results'/'capability_latent_matrix_v2.csv'; BASE=ROOT/'results'/'phase_v_h1_persistence_bars_v2.csv'; OUT=ROOT/'results'/'phase_v_persistence_correspondence_v1.json'
...
with BASE.open(newline='',encoding='utf-8') as f: base=[(float(r['birth']),float(r['death'])) for r in csv.DictReader(f) if r['death']!='inf']
assert len(base)==55
def diagram(Y):
 ...
 simplices=[tuple(c) for k in range(1,n+1) for c in itertools.combinations(range(n),k)]; simplices.sort(key=lambda s:(filt(s),len(s),s)); index={s:i for i,s in enumerate(simplices)}; values=[filt(s) for s in simplices]
...
def hungarian(cost):
 N=len(cost); u=[0.0]*(N+1); v=[0.0]*(N+1); q=[0]*(N+1); way=[0]*(N+1)
 for i in range(1,N+1):
  q[0]=i; j0=0; minv=[float('inf')]*(N+1); used=[False]*(N+1)
  while True:
   used[j0]=True; i0=q[j0]; delta=float('inf'); j1=0
   ...
   for j in range(N+1):
    if used[j]: u[q[j]]+=delta; v[j]-=delta
    else: minv[j]-=delta
   j0=j1
   if q[j0]==0: break
  while True:
   j1=way[j0]; q[j0]=q[j1]; j0=j1
   if j0==0: break
 ans=[0]*N
 for j in range(1,N+1): ans[q[j]-1]=j-1
 return ans
ranges=[max(X[i][j] for i in range(n))-min(X[i][j] for i in range(n)) for j in range(p)]; levels=[0.01,0.02,0.05]; audits=[]
for level in levels:
 Y=[[x+(-1.0 if ((i+2*j)%2) else 1.0)*level*ranges[j] for j,x in enumerate(row)] for i,row in enumerate(X)]; pert=diagram(Y); assert len(pert)==len(base)==55
 cost=[[max(abs(a[0]-b[0]),abs(a[1]-b[1])) for b in pert] for a in base]; match=hungarian(cost); pairs=[]
 ...
 disps=[x['linf_displacement'] for x in pairs]
 audits.append({'level':level,'n_matched':len(pairs),'max_linf_displacement':max(disps,default=0.0),'mean_linf_displacement':sum(disps)/len(disps) if disps else 0.0,'pairs':pairs})
