...
def main():
    ...
    text.write_text(
        ...
        "where r0=B0-sum_i g_i, r1=-sum_i g_i(a_i+d_i), and\n"
        "r2=-sum_i g_i a_i d_i.\n\n"
        "For marginal cell m, F=d_m and A=sum_i w_i g_i, B=sum_i w_i g_i a_i,\n"
        "C=r0, D=r1, E=r2. Hence Pi(lambda)=A+B lambda+\n"
        "(C+D lambda+E lambda^2)/(1+F lambda).\n\n"
        "The primitive restrictions include g_i >= 0, w_i >= 0, kappa_i^0 in [0,1],\n"
        "and c_i^0 > 0. Therefore a_i in [-1,0], d_i > -1, and F > -1.\n"
        "The first-order numerator satisfies p0=B+D-FC, p1=2(BF+E),\n"
        "p2=F(BF+E). Put S=BF+E and T=E-FD+F^2 C. Then\n"
        "Delta_P=4ST=2(BF+E)q0, q0=2T. Moreover\n"
        "Delta_Q=4F^4(r1^2-4r0 r2).\n\n"
        "Also q2=-2F^2 sum_i g_i a_i d_i. Thus if every binding cell has\n"
        "c_i^0 >= 1 then q2 >= 0, while if every binding cell has c_i^0 <= 1\n"
        "then q2 <= 0. These identities expose the primitive inequalities needed\n"
        "for parameter-free monotonicity and curvature theorems.\n",
        encoding="utf-8")
    print(out)
